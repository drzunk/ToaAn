...
public class XemLaiGuiDonPage {
    ...
    public void waitStepReady() {
        webUI.waitUntilVisible(stepReadyMarker, 15, "Bước 6 [Xem lại & Gửi đơn]");
    }
    ...
    /**
     * Bấm [Chỉnh sửa] tại một mục trên màn Xem lại và chờ form bước tương ứng hiển thị.
     */
    public void clickChinhSua(String tenMuc, By stepMarker, String moTaBuoc) {
        ...
        webUI.waitUntilVisible(stepMarker, 15, moTaBuoc);
    }
    ...
    public void clickGuiDon() {
        webUI.waitUntilVisible(btnGuiDon, 10, "Nút [Gửi đơn]");
    ...
    public void choGuiDonThanhCong() {
        System.out.println(" ⏳ Chờ hệ thống xử lý gửi đơn...");
        for (int i = 1; i <= 30; i++) {
            if (webUI.existsNow(successAny)) {
                System.out.println(" ✅ Gửi đơn thành công (" + i + "s).");
                return;
            }
            if (!webUI.existsNow(btnGuiDon) && i >= 3) {
                System.out.println(" ✅ Đã chuyển khỏi màn hình Gửi đơn (" + i + "s).");
                return;
            }
            if (i == 1 || i % 5 == 0) {
                System.out.println(" ⏳ Chờ phản hồi gửi đơn... (" + i + "/30s)");
            }
            webUI.sleep(1);
        }
        throw new RuntimeException("❌ Timeout: Không nhận được phản hồi thành công sau khi Gửi đơn (30s).");
    }
