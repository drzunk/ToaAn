...
public class NguyenDonPage {
    ...
    public void dienThongTinLienHe(String thuongTru, String lienLac, String sdt, String email) {
        ...
        if (yeuCauGiong) {
            ...
            webUI.waitUntilVisible(txtDiaChiLienLac, WaitConfig.FIELD, "Ô [Địa chỉ liên lạc]");
    ...
    public void chonNguoiDaiDien(String coNguoiDaiDien, String tenNguoiDaiDien, String quanHe) {
        // Hộp kiểm này chỉ có trên form Cá nhân — Tổ chức đã có "Người đại diện pháp luật"
        if (!webUI.existsNow(chkNguoiDaiDien) && !webUI.isElementVisible(chkNguoiDaiDien)) {
            System.out.println(" ⏩ Bỏ qua [Người đại diện pháp lý] — không có trên biểu mẫu (vd. Tổ chức).");
            TestActionLog.boQua("Người đại diện pháp lý", "Không có trên biểu mẫu (vd. Tổ chức)");
            return;
        }
    ...
    public void chonTuCachNguoiNopDon(String tuCach) {
        By btn = By.xpath(
                "//label[contains(., 'Tư cách người nộp đơn')]/following-sibling::div//button"
                        + " | //label[contains(., 'Tư cách người nộp đơn')]/ancestor::div[contains(@class,'space') or contains(@class,'grid') or contains(@class,'flex')][1]//button[contains(., 'Chọn') or contains(., '—')]");
