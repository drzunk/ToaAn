...
- Use `/fis-scout ext` or `scout` agent for codebase search
...
**Fast:**
- Use `/fis-plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/fis-plan --parallel` for dependency graph + file ownership matrix
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/fis-plan validate` skill invocation
...
- Use `fis:ai-multimodal` for image assets
...
Read thresholds from `.fisrc.json` (`simplify.threshold.{locDelta,fileCount,singleFileLoc}`),
...
Skip the step entirely when `FIS_SIMPLIFY_DISABLED=1` or
`.fisrc.json` `simplify.gate.enabled` is `false`.
...
Edit `plan.md` directly to update phase status — only change the Status column cell, preserve table structure exactly.

Update `plan.md` frontmatter `status` field: `pending` → `in-progress` → `completed` based on checkbox state.
