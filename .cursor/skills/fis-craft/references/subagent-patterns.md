...
Standard patterns for spawning and using subagents in craft workflows.
...
```
- Use `/fis-scout ext` (preferred) or `/fis-scout` (fallback)
...
- Trigger when live `git diff --numstat HEAD --ignore-all-space` breaches any
  `simplify.threshold` from `.fisrc.json` (defaults: 400 LOC / 8 files / 200 single-file LOC)
...
- Skip when `FIS_SIMPLIFY_DISABLED=1` or `.fisrc.json` `simplify.gate.enabled=false`
