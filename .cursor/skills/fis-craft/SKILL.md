...
# Craft - Smart Feature Implementation
...
**Example:**
```
/fis-craft "Add user authentication to the app" --fast
/fis-craft path/to/plan.md --auto
...
- **Acceptance criteria:** Every acceptance criterion from the issue / PR must be covered by a passing test or manual proof before finalizing.
...
| Scout | `fis:scout` | Optional in code |
...
**Typically follows:** `/fis-plan` (execute a plan), `/fis-brainstorm` (implement agreed solution)
**Typically precedes:** `/fis-code-review` (review after implementation), `/fis-test` (validate changes)
**Related:** `/fis-fix` (alternative for bug fixes), `/fis-plan` (create plan before cooking)
