...
Use `fis:agent-browser` for browser testing, screenshots, form fills, scraping, exploratory QA, cloud browsers, Electron apps, and flows where a fresh or tool-managed browser is fine.

Use `fis:chrome-profile` instead when the task needs the user's actual Chrome profile: existing cookies, logged-in sessions, a specific Google account, a tenant/workspace already open in daily Chrome, or deterministic targeting across multiple Chrome profiles.
...
For real user Chrome automation with profile/cookie state, use `fis:chrome-profile`. For low-level Chrome DevTools Protocol diagnostics, use `chrome-devtools-mcp` via `/fis-use-mcp`.
