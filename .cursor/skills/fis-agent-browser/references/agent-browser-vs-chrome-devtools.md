...
Use `fis:agent-browser` for normal browser automation and testing when a fresh or tool-managed browser is acceptable. Use `fis:chrome-profile` only when the task needs the user's actual Chrome profile state.
