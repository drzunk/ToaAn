...
# fis:predict — Multi-Persona Pre-Analysis
...
- Trivial or low-risk changes (use `fis:debug` for bugs, `fis:plan` for already-decided tasks)
...
|---------------|-------|-----|
| Deepen risk scenarios | `fis:scenario` | Feed Risk Summary rows as feature description |
| Create implementation plan | `fis:plan` | Attach Recommendations as constraints to planner |
| High-risk feature implementation | `fis:craft` | Reference CAUTION/STOP items as acceptance gates |
...
```
/fis-predict "Add WebSocket support for real-time notifications"
/fis-predict "Migrate authentication from JWT to session cookies"
/fis-predict "Add multi-tenancy to the database layer"
/fis-predict "Replace REST API with GraphQL" --files src/api/**/*.ts
```
