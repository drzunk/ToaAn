...
## Integration with Craft/Planning

Scout tasks are **independent** from craft/planning phase tasks — NOT parent-child.

**Rationale:** Different lifecycle. Scout completes before craft continues. Mixing creates confusion in TaskList.

**Sequence when craft spawns scout:**
1. Craft Step 2 → spawns planner → planner spawns scout
2. Scout registers its own tasks (Step 3), executes (Step 4-5)
3. Scout returns aggregated report → planner continues
4. Craft Step 3 hydrates phase tasks (separate from scout tasks)
