...
Not for: full project cloning (`fis:bootstrap`), simple file copy, or package installation.

## Usage

```text
/fis-port-feature <github-url|owner/repo|local-path> [feature-description] [--compare|--copy|--improve|--port] [--auto|--fast]
```
...
1. Pack the source with `/fis-repomix`.
...
4. Use `/fis-scout` on the local project to map architecture, similar features, and integration points.
...
- activate `/fis-sequential-thinking` to trace multi-step flows
...
Do not invoke `/fis-brainstorm` from inside `port-feature`; that skill can create its own planning handoff and break `port-feature`'s phase ownership.
...
- `port-feature` is a front door, not a second orchestration stack. Keep planning and delivery ownership in `fis-plan` and `fis-craft`.
...
- other modes: present the plan path and hand implementation to `/fis-craft`

Implementation handoff text:

```text
Plan ready at ./plans/<plan-dir>/plan.md. To implement, run /fis-craft <plan-path>.
```
