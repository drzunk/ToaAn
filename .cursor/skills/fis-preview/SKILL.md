...
### View Mode
- `/fis-preview <file.md>` - View markdown file in novel-reader UI
- `/fis-preview <directory/>` - Browse directory contents
- `/fis-preview --stop` - Stop running server

### Generation Mode (Markdown)
- `/fis-preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/fis-preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/fis-preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/fis-preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/fis-preview --html --explain <topic>` - Self-contained HTML explanation
- `/fis-preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/fis-preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/fis-preview --html --diff [ref]` - Visual diff review
- `/fis-preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/fis-preview --html --recap [timeframe]` - Project context snapshot
...
| Flag without topic | Ask user: "Please provide a topic: `/fis-preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/fis-preview --stop` first |
...
Use `/fis-mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/fis-ui-ux-pro-max` for richer style selection
