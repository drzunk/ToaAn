...
```text
/fis-port-repo <github-url|owner/repo|local-path> [feature-description] [--compare|--copy|--improve|--port] [--auto|--fast]
```
...
Do not invoke `/fis-brainstorm` from inside `port-repo`; that skill can create its own planning handoff and break `port-repo`'s phase ownership.
...
- `port-repo` is a front door, not a second orchestration stack. Keep planning and delivery ownership in `fis-plan` and `fis-craft`.
