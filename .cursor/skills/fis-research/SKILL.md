...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.claude/.fisrc.json` (or `~/.claude/.fisrc.json`) for `skills.research.useGemini` (default: `false`). If `false` or absent, skip Gemini and use WebSearch directly.
   - **Gemini Model**: Read from `.claude/.fisrc.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
2. **Deep Content Analysis**:
   - When you found a potential Github repository URL, use `fis:docs-seeker` skill to find read it.
