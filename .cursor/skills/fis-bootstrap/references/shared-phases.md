...
These phases apply after planning is complete and craft skill is activated.
Craft skill handles most of these — this reference documents bootstrap-specific guidance.

## Implementation

Handled by **fis:craft** skill. Bootstrap-specific notes:
...
- Asset pipeline: `fis:ai-multimodal` (generate/analyze) → `imagemagick` (crop/resize) → background removal if needed
