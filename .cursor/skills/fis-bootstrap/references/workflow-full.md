...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `fis:ai-multimodal` skill generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `fis:ai-multimodal` skill
5. Screenshot wireframes with `fis:chrome-devtools` → save to `./docs/wireframes/`
...
Activate **fis:plan** skill: `/fis-plan --hard <requirements>`
...
Activate **fis:craft** skill: `/fis-craft <plan-path>` (interactive mode — review gates at each step)
