...
3. See `task-management.md` for patterns and craft handoff protocol
...
2. **Write `plan.md`** using the Write tool — follow the schema below.

3. **Write each `phase-XX-<name>.md`** using the Write tool:
   - Architecture, implementation steps, success criteria
   - Requirements, risk assessment, security considerations

4. **Update phase status** by editing the Status column cell in `plan.md` directly — preserve table structure exactly.

**IMPORTANT:** Always Read an existing file before Write. Claude Code enforces Read-before-Write; skipping it wastes the Write's input tokens and forces a retry.
...
- Read `plan.md` directly to inspect resolved dependency state.
