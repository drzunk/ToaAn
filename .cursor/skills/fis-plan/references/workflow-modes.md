...
4. **Context reminder:** `/fis-craft --auto {absolute-plan-path}/plan.md`

**Why `--auto` craft flag?** Fast planning pairs with fast execution — skip review gates.
...
2. Read codebase docs; if stale/missing: run `/fis-scout` to search codebase
...
7. **Context reminder:** `/fis-craft {absolute-plan-path}/plan.md`

**Why no craft flag?** Thorough planning needs interactive review gates.
...
2. Read all relevant docs and use `/fis-scout` across the affected areas
...
8. Output the standard `/fis-craft {absolute-plan-path}/plan.md` reminder
...
7. **Context reminder:** `/fis-craft --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `/fis-plan red-team {plan-directory-path}`.
```
/fis-plan red-team {plan-directory-path}
```
...
| `auto` | Run `/fis-plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
/fis-plan validate {plan-directory-path}
```
...
| Mode | Craft Command |
|------|-----------------------------|
| fast | `/fis-craft --auto {path}/plan.md` |
| hard | `/fis-craft {path}/plan.md` |
| deep | `/fis-craft {path}/plan.md` |
| parallel | `/fis-craft --parallel {path}/plan.md` |
| two | `/fis-craft {path}/plan.md` |

If planning ran with `--tdd`, append `--tdd` to the reminder above so craft keeps
the tests-first execution path. Example:
`/fis-craft {path}/plan.md --tdd`

> **Best Practice:** Run `/clear` before implementing to reduce planning-context carryover.
> Then run the craft command above.
