...
| Plan will be executed by craft | Yes | Seamless handoff |
...
## Craft Handoff Protocol

### Same-Session (planning → craft immediately)

1. Planning hydrates tasks → tasks exist in session
2. Craft Step 3: `TaskList` → finds existing tasks → picks them up
3. Craft skips re-creation, begins implementation directly

### Cross-Session (new session, resume plan)

1. User runs `/fis-craft path/to/plan.md` in new session
2. Craft Step 3: `TaskList` → empty (tasks died with old session)
3. Craft reads plan files → re-hydrates from unchecked `[ ]` items
4. Already-checked `[x]` items = done, skip those

### Sync-Back (craft Step 6)
