...
```bash
node .claude/skills/fis-worktree/scripts/worktree.cjs info --json
```
...
**Monorepo:**
```bash
node .claude/skills/fis-worktree/scripts/worktree.cjs create "<PROJECT>" "<SLUG>" --prefix <TYPE>
```

**Standalone:**
```bash
node .claude/skills/fis-worktree/scripts/worktree.cjs create "<SLUG>" --prefix <TYPE>
```
...
**Typically precedes:** `/fis-craft` (implement in worktree), `/fis-fix` (fix in worktree)
