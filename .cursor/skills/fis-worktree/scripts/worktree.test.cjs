 ...
 * Run: node .claude/skills/fis-worktree/scripts/worktree.test.cjs
 */
...
const MONOREPO_DIR = '/home/kai/fis-ai-kit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'fis-ai-kit-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from fis-ai-kit-engineer submodule
  const submodulePath = '/home/kai/fpt-fis/fis-ai-kit-engineer';
...
test('create with partial project match in monorepo', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'fis-ai-cli', 'Should find fis-ai-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'fis-ai-kit' matches multiple projects
  const result = run('create fis-ai-kit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
