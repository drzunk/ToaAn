 ...
 *   5. "fis-ai-kit-default"     last resort fallback
 */
...
const QUOTA_DIR = path.join(os.homedir(), ".fis-ai-kit");
...
if (directProjectId) {
  ...
  projectId = detected || "fis-ai-kit-default";
  resolvedProjectName = projectId;
  isNameBasedProject = true;
  if (detected) {
...
async function main() {
  try {
    ...
    if (remaining < creditsNeeded) {
      ...
      console.error("[i] Use fis:ui-ux-pro-max as fallback, or wait until midnight UTC.");
      process.exit(2);
    }
    ...
    if (err.code === "RATE_LIMITED") {
      ...
      q.count = q.limit;
      saveQuota(q);
      console.error("[X] Daily quota exceeded (local tracker synced). Try tomorrow or use fis:ui-ux-pro-max.");
