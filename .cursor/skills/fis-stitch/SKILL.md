...
```bash
cd ~/.claude/skills/fis-stitch/scripts && npm install
```
...
```bash
# In ~/.claude/.env
STITCH_PROJECT_ID="my-project"    # Default project (auto-creates "fis-ai-kit-default" if unset)
...
1. **Check quota** — Run `stitch-quota.ts check`. If exhausted, suggest `fis:ui-ux-pro-max` fallback.
...
6. **Implement** — Hand off exported artifacts to implementation skill:
   - `fis:frontend-design` — React/Vue/Svelte components from Tailwind export
   - `fis:ui-ux-pro-max` — Full page layouts with style guide integration
   - `fis:ui-styling` — Design token extraction from DESIGN.md
...
- Local tracking via `~/.fis-ai-kit/.stitch-quota.json`
- Warns when remaining credits < 20%
- **Fallback:** When exhausted, use `fis:ui-ux-pro-max` for text-based design generation
