...
Orchestration workflows chaining Stitch design generation with FIS AI Kit implementation skills.
...
# 4. Activate fis:frontend-design to implement React components from the spec
```
...
|-------------|-------|-------------|
| React/Vue/Svelte components | `fis:frontend-design` | Converts Tailwind HTML to framework components |
| Full page with style guide | `fis:ui-ux-pro-max` | Builds complete layouts with design system |
| Design token extraction | `fis:ui-styling` | Extracts shadcn/Tailwind tokens from DESIGN.md |

## Handoff Protocol

1. `fis:stitch` exports artifacts to `./stitch-exports/` (or plan directory)
...
# -> Skip Stitch, use fis:ui-ux-pro-max for text-based design instead
