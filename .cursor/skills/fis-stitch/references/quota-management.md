...
Stitch SDK has no programmatic quota check endpoint. FIS AI Kit tracks locally:

**File:** `~/.fis-ai-kit/.stitch-quota.json`
...
2. Skill prints: "Daily quota exhausted. Use fis:ui-ux-pro-max as fallback."
3. Activate `fis:ui-ux-pro-max` with the same design prompt
...
Local tracking can drift if user generates designs outside FIS AI Kit (via Stitch web UI or other tools). If you hit `RATE_LIMITED` error despite local tracker showing credits available:
