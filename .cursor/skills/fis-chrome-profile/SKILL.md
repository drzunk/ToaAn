...
Profile-aware browser automation path for FIS AI Kit when the agent needs the user's real Chrome state: the right Google account, cookies, workspace, tenant, or logged-in product session.

This is not the default for ordinary browser testing. Use `fis:agent-browser` when a fresh or tool-managed browser is enough. Use project-native Playwright or `fis:web-testing` for repeatable CI tests. Use this skill when the user's actual Chrome profile state matters.
...
```bash
bash .claude/skills/fis-chrome-profile/scripts/install.sh
```
...
That per-machine config survives FIS AI Kit updates. Use `chrome-profile setup --yes` for non-interactive bootstrap.
