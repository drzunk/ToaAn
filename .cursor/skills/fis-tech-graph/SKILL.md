...
description: >-
  ...
  Pairs with /fis-preview --diagram for visual self-review and /fis-mermaidjs-v11
...
> Vendored from upstream `fireworks-tech-graph` (yizhiyanhua-ai, MIT). System dep `librsvg` (provides `rsvg-convert`) is installed via `claude/skills/install.sh`. Do NOT use upstream's `npx skills add` instructions — this skill is part of the FIS AI Kit stack.

## Vendoring Notes

This skill is vendored — installation is handled by `claude/skills/install.sh` (no manual `npx skills add` step). The required system dependency `librsvg` (`rsvg-convert` binary) is listed in `claude/skills/install.sh` alongside FFmpeg/ImageMagick; run that script manually or via DAI's kit install flow to provision it. Drift from upstream is tracked in `~/fis-ai-kit/.maintainer/external-sources.json` (Mode A entry) — see CLAUDE.md "External Source Drift Watcher" in the monorepo for the update procedure.
