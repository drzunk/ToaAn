...
1. **`/fis-design (brand mode)`** — Extract brand colors, fonts, voice from user's project
2. **`/fis-design (system mode)`** — Get design tokens (spacing, typography scale, color palette)
3. **Randomly invoke ONE of:** `/fis-ui-ux-pro-max` OR `/fis-frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
