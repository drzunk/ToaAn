...
name: fis:drawio
category: utilities
keywords: [drawio, diagram, flowchart, architecture, bpmn, erd, uml, c4, sequence, swimlane]
...
metadata:
  author: fis-ai-kit
  version: "1.0.0"
  upstream: "drawio-skill 1.34.0 (MIT © Agents365-ai)"
---

# Draw.io Diagrams

Kit default engine for process / workflow / architecture / BPMN / ERD diagrams — output `.drawio` plus an exported PNG/SVG embedded in Markdown. For lightweight diagrams-as-code that must render **inline** in Markdown/HTML, use `/fis-mermaidjs-v11` instead.
...
- A casual hand-drawn / whiteboard look → `/fis-excalidraw`.
- Lightweight diagrams-as-code that must render **inline** in Markdown/HTML (no export step) → `/fis-mermaidjs-v11`.
- Freeform infinite-canvas sketching or freehand strokes → `/fis-excalidraw`.
...
**Regular (non-dev) users:** DAI installs this skill's files but not the draw.io desktop app. On dev machines the kit's `install.sh` (macOS/Linux) / `install.ps1` (Windows) provision draw.io + Graphviz automatically. On a plain machine, install draw.io once with the per-OS commands below (or the installer). Without it, the skill still emits a `.drawio` file plus a diagrams.net browser link (see **Fallback chain**) — no local export, but the diagram is still usable.
