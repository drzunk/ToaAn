...
ultrathink
Activate `fis:frontend-design` skill to create a showcase HTML presentation for the following request:
...
Invoke `/fis-project-management` **first**, before reading/analyzing the request or doing any other work. This skill owns plan/task lifecycle; `show-off` is a consumer.
...
- Activate `fis:frontend-design` skill to create a stunning HTML file:
...
- Capture each section as images (JPG/PNG) at `assets/showoff/<mission-name>/images/` with ratio-based prefix (`horizontal`, `vertical`, `square`).
  ...
  **IMPORTANT:** Use the parallel capture script for efficiency:
  ```bash
  node .claude/skills/fis-show-off/scripts/capture-sections.js \
