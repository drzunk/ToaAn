...
Entry point for using the `fis:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `fis:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `fis:frontend-design` with `fis:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `fis:chrome-devtools` Skill
Use `fis:chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `fis:media-processing` Skill
