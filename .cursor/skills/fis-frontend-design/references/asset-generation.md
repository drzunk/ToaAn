...
Complete workflow for generating design-aligned visual assets using `fis:ai-multimodal` skill.
...
Use `fis:ai-multimodal` skill's image generation capabilities:
...
Use `fis:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `fis:media-processing` skill for post-generation adjustments
