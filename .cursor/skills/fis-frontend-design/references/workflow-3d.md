...
## Prerequisites
- Activate `fis:ui-ux-pro-max` skill first
- Activate `fis:threejs` skill for 3D and WebGL expertise
- Have `fis:ai-multimodal` skill ready for asset generation

## Initial Research
Run `fis:ui-ux-pro-max` searches:
```bash
python3 .claude/skills/fis-ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 .claude/skills/fis-ui-ux-pro-max/scripts/search.py "immersive 3d" --domain style
python3 .claude/skills/fis-ui-ux-pro-max/scripts/search.py "animation" --domain ux
```
...
Use `fis:ai-multimodal` skill for:
- Textures and materials
- Skyboxes and environment maps
- Particle sprites
- Video backgrounds

Use `fis:media-processing` skill for:
