...
- **Companion skill** — a `/fis:*` skill discoverable on the Claude Plugins Marketplace
...
Scope: converting existing code into CLI and/or MCP. Not for: building a server from scratch (use `/fis-mcp-builder`), raw npm scaffolding, or publishing without an agent-use story.

## Usage

```text
/fis-agentize [feature-or-module] [--both|--mcp|--cli] [--auto|--ask]
```
...
Invoke `/fis-project-management` **before** touching code. This skill owns plan/task lifecycle; `agentize` is a consumer.
...
- Set the active plan context so downstream skills (`fis-scout`, `fis-plan`, `fis-craft`, `fis-test`, `fis-docs`, `fis-skill-creator`) write into the same plan folder.
...
Invoke `/fis-scout` to understand the target codebase. Without this, everything downstream is guessed.
...
1. **Tests** — invoke `/fis-test` to generate:
...
3. **Docs** — invoke `/fis-docs` to generate:
...
4. **Companion skill** — invoke `/fis-skill-creator` to generate:
...
Next: /fis-craft <plan-path> to execute any remaining implementation.
```
