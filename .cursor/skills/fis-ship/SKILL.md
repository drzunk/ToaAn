...
Step 8:  Journal          → Write technical journal via /fis-journal
...
Step 12: Create PR/MR     → gh pr create (GitHub) or glab mr create (GitLab, incl. self-hosted), auto-detected from remote
```
...
User says `/fis-ship` → run full pipeline → output PR URL.
...
**Typically follows:** `/fis-code-review` (ship after review passes)
**Typically precedes:** `/fis-journal` (document after shipping)
**Related:** `/fis-code-review` (review before shipping), `/fis-test` (test before shipping)
