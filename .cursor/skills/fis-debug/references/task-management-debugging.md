...
Debug tasks are **separate from** craft/planning phase tasks.

**When craft spawns debugger:**
1. Craft encounters failing tests → creates debug pipeline tasks
2. Debug pipeline executes (assess → collect → analyze → fix → verify)
3. All debug tasks complete → craft marks phase debugging as done
4. Craft proceeds to next phase
