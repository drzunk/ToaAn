...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `fis:chrome-devtools` skill fallback.
...
When Chrome MCP is not configured, use `fis:chrome-devtools` skill (Puppeteer with bundled Chromium):

```bash
SKILL_DIR="$HOME/.claude/skills/fis-chrome-devtools/scripts"
...
If `fis:chrome-devtools` skill is also unavailable, skip visual verification and note in report:
