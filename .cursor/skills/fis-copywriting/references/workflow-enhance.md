...
1. **Screenshots provided** → Use `fis:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `fis:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/fis-scout ext` (preferred) or `/fis-scout` to find files needing enhancement
