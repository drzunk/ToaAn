...
# fis:scenario — Edge Case & Scenario Explorer
...
| Next Step | Skill | How |
|-----------|-------|-----|
| Generate test cases from scenarios | `fis:test` | Pass scenario table as input context |
| Inform implementation plan risks | `fis:plan` | Paste Critical/High rows into risk assessment |
| Deep persona debate on top risks | `fis:predict` | Feed Critical scenarios as the change proposal |
...
```
/fis-scenario src/api/payment.ts
/fis-scenario "User registration with OAuth providers"
/fis-scenario src/middleware/auth.ts
/fis-scenario "Add multi-tenancy to the database layer"
```
