...
Browser-based visual testing via `fis:chrome-devtools` skill. Screenshots, responsive checks, accessibility audits, form automation, console error collection. Includes auth injection for protected routes.
...
6. For frontend: run UI tests via `fis:chrome-devtools` skill
...
- **Browser**: `fis:chrome-devtools` skill for UI testing (screenshots, ARIA, console, network)
- **Analysis**: `fis:ai-multimodal` skill for screenshot analysis
- **Debugging**: `fis:debug` skill when tests reveal bugs requiring investigation
- **Thinking**: `fis:sequential-thinking` skill for complex test failure analysis
...
**Typically follows:** `/fis-craft` (test after implementation), `/fis-fix` (test after bug fix)
**Typically precedes:** `/fis-code-review` (review after tests pass)
**Related:** `/fis-craft` (implement then test), `/fis-fix` (fix then test)
