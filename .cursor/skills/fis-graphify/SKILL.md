...
related: [fis:repomix, fis:scout, fis:gkg]
maturity: beta
metadata:
  author: safishamsi
...
/fis-scout "auth module"           # find specific files
```
...
**Typically precedes:** `/fis-plan` (understand architecture before planning)
**Related:** `/fis-scout` (quick file search), `/fis-repomix` (full context dump), `/fis-gkg` (semantic symbol navigation)
