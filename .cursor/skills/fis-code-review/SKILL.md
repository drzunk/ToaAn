...
| **Checklist review** | Pre-landing, `/fis-ship` pipeline, security audit | `references/checklist-workflow.md` |
...
**Process:**
1. Invoke `/fis-scout` with edge-case-focused prompt
...
- **Craft Handoff:** Craft completes phase → review pipeline tasks (incl. adversarial) → all complete → craft proceeds
...
|------------|-----------|---------|
| `/fis-code-review codebase` | `references/codebase-scan-workflow.md` | Scan & analyze the codebase |
| `/fis-code-review codebase parallel` | `references/parallel-review-workflow.md` | Ultrathink edge cases, then parallel verify |
...
**Typically follows:** `/fis-craft` (review after implementation), `/fis-fix` (review after bug fix)
**Typically precedes:** `/fis-ship` (ship after review passes)
**Related:** `/fis-scout` (scout before reviewing), `/fis-test` (test before reviewing)
