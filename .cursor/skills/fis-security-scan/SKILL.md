...
```
/fis-security-scan              # Full scan of current project
/fis-security-scan --secrets-only   # Only secret/credential detection
/fis-security-scan --deps-only      # Only dependency audit
/fis-security-scan src/api/         # Scan specific directory
```
...
If `--auto` mode active in craft workflow: save report to `{FIS_REPORTS_PATH}` or `plans/reports/security-scan-{date}.md`.
