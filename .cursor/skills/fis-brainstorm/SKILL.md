...
- Use `fis:docs-seeker` skill to read latest documentation of external plugins/packages
- Leverage `fis:ai-multimodal` skill to analyze visual materials and mockups
- Query `psql` command to understand current database structure and existing data
- Employ `fis:sequential-thinking` skill for complex problem-solving that requires structured analysis
...
flowchart TD
    ...
    H --> I{Create Plan?}
    I -->|Yes| J[Invoke /fis-plan]
    I -->|No| K[End Session]
    J --> L[Journal]
    K --> L
```

**This diagram is the authoritative workflow.** If prose conflicts with this flow, follow the diagram. The terminal state is either `/fis-plan` or end.

## Your Process
1. **Scout Phase**: Use `fis:scout` skill to discover relevant files and code patterns, read relevant docs in `<project-dir>/docs` directory, to understand the current state of the project
...
9. **Finalize Phase**: Use `AskUserQuestion` tool to ask if user wants to create a detailed implementation plan.
   - If `Yes`: Run `/fis-plan` command with the brainstorm summary context as the argument to ensure plan continuity.
     **CRITICAL:** The invoked plan command will create `plan.md` with YAML frontmatter including `status: pending`.
   - If `No`: End the session.
10. **Journal Phase**: Run `/fis-journal` to write a concise technical journal entry upon completion.
...
## Output Requirements
**IMPORTANT:** Invoke "/fis-project-organization" skill to organize the reports.
...
**Typically follows:** `/fis-debug` (brainstorm solutions for diagnosed issues), `/fis-scout` (brainstorm after discovery)
**Typically precedes:** `/fis-plan` (plan the agreed solution)
**Related:** `/fis-plan` (plan after brainstorming), `/fis-debug` (debug before brainstorming)
