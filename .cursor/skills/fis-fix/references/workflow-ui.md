...
1. `fis:ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `fis:ui-ux-pro-max` - Design principles
3. `fis:frontend-design` - Implementation patterns

## Pre-fix Research
```bash
python3 .claude/skills/fis-ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 .claude/skills/fis-ui-ux-pro-max/scripts/search.py "<style>" --domain style
python3 .claude/skills/fis-ui-ux-pro-max/scripts/search.py "accessibility" --domain ux
```
...
Analyze screenshots/videos with `fis:ai-multimodal` skill.
...
Screenshot + `fis:ai-multimodal` analysis.
...
## Tips
- Use `fis:ai-multimodal` for generating visual assets
