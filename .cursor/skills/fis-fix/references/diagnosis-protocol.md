...
Read, don't assume. Use `fis:debug` (systematic-debugging Phase 1).
...
Activate `fis:sequential-thinking` skill. Form hypotheses through structured reasoning.
...
Use `fis:debug` (root-cause-tracing technique). Trace backward:
...
1. Auto-activate `fis:problem-solving` skill
