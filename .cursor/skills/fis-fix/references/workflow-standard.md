...
2. Activate `fis:debug` skill. Use `debugger` subagent if needed.
3. Activate `fis:sequential-thinking` — form hypotheses through structured reasoning.
...
- Apply `fis:problem-solving` skill if stuck
- Use `fis:sequential-thinking` for complex logic
...
- Activate `fis:project-management` for task sync-back and plan status updates
...
| 2 | `fis:debug`, `fis:sequential-thinking`, `debugger` subagent, parallel `Explore`, (`fis:problem-solving` auto) |
| 3 | `fis:problem-solving` (if stuck), `fis:sequential-thinking` (complex logic) |
| 4 | `tester` subagent, parallel `Bash` verification |
| 5 | `code-reviewer` subagent |
| 6 | `fis:project-management`, `git-manager`, `docs-manager` subagents |
