...
**Mandatory:** Activate `fis:scout` skill or launch 2-3 `Explore` subagents in parallel:
```
...
2. Activate `fis:debug` skill (systematic-debugging + root-cause-tracing).
3. Activate `fis:sequential-thinking` — structured hypothesis formation.
4. Spawn parallel `Explore` subagents to test each hypothesis.
5. If 2+ hypotheses fail → auto-activate `fis:problem-solving`.
...
Implement per plan. Use `fis:context-engineering`, `fis:sequential-thinking`, `fis:problem-solving`.
...
- Activate `fis:project-management` for task sync-back, plan status updates, and progress tracking
...
|------|------------------|
| 1 | `fis:scout` OR parallel `Explore` subagents |
| 2 | `fis:debug`, `fis:sequential-thinking`, parallel `Explore`, (`fis:problem-solving` auto) |
| 3 | `researcher` (runs parallel with steps 1+2) |
| 4 | `fis:brainstorm` |
| 5 | `planner` |
| 6 | `fis:problem-solving`, `fis:sequential-thinking`, `fis:context-engineering` |
| 7 | `tester`, parallel `Bash` verification |
| 8 | `code-reviewer` |
| 9 | `fis:project-management`, `docs-manager`, `git-manager` |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `chrome`, `fis:chrome-devtools` or any relevant skills/tools to verify.
**Visual Assets:** Use `fis:ai-multimodal` for visual assets generation, analysis and verification.
