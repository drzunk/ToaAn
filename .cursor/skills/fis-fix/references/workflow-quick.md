...
Activate `fis:debug` skill. Activate `fis:sequential-thinking` for structured analysis.
...
|------|------------------|
| 1 | `fis:scout` (minimal) or direct file read |
| 2 | `fis:debug`, `fis:sequential-thinking` |
| 3 | Parallel `Bash` for verification |
| 4 | `code-reviewer` subagent |
| 5 | `git-manager` subagent |

**Extra:** `fis:context-engineering` if dealing with AI/LLM code
