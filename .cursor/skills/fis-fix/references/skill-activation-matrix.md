...
|------------|------|--------|
| `fis:scout` OR parallel `Explore` | Step 1 | Understand codebase context before diagnosing |
| `fis:debug` | Step 2 | Systematic root cause investigation |
| `fis:sequential-thinking` | Step 2 | Structured hypothesis formation — NO guessing |
...
|-------|------------------------|
| `fis:problem-solving` | 2+ hypotheses REFUTED in Step 2 diagnosis |
| `fis:sequential-thinking` | Always in Step 2 (mandatory for hypothesis formation) |

## Conditional Activation

| Skill | Activate When |
|-------|---------------|
| `fis:brainstorm` | Multiple valid fix approaches, architecture decision (Deep only) |
| `fis:context-engineering` | Fixing AI/LLM/agent code, context window issues |
| `fis:ai-multimodal` | UI issues, screenshots provided, visual bugs |
| `fis:project-management` | Moderate+ workflows — task hydration, sync-back, progress tracking |
...
| `fis:code-review` | After fix, verify quality and security (Step 5) |
...
|----------|------------------|
| Quick | `fis:scout` (minimal), `fis:debug`, `fis:sequential-thinking`, `fis:code-review`, parallel `Bash` verification |
| Standard | Above + Tasks, `fis:problem-solving` (auto), `fis:project-management`, `tester`, parallel `Explore` |
| Deep | All above + `fis:brainstorm`, `fis:context-engineering`, `researcher`, `planner` |
| Parallel | Per-issue Task trees + `fis:project-management` + `fullstack-developer` agents + coordination via `TaskList` |
...
| Step 1: Scout | `fis:scout` OR 2-3 parallel `Explore` → map files, deps, tests |
| Step 2: Diagnose | Capture pre-fix state → `fis:debug` → `fis:sequential-thinking` → parallel `Explore` hypotheses → (`fis:problem-solving` if 2+ fail) |
...
| Step 6: Finalize | Report → `docs-manager` → `TaskUpdate` → `git-manager` → `/fis-journal` |

## Detection Triggers

| Keyword/Pattern | Skill to Consider |
|-----------------|-------------------|
| "AI", "LLM", "agent", "context" | `fis:context-engineering` |
| "stuck", "tried everything" | `fis:problem-solving` |
| "complex", "multi-step" | `fis:sequential-thinking` |
| "which approach", "options" | `fis:brainstorm` |
| "latest docs", "best practice" | `researcher` subagent |
| Screenshot attached | `fis:ai-multimodal` |
