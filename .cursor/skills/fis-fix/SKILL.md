...
See `references/selection-workflow.md` for AskUserQuestion format.
...
1. Activate `fis:scout` skill OR launch 2-3 parallel `Explore` subagents
...
2. Activate `fis:debug` skill (systematic-debugging + root-cause-tracing techniques).
3. Activate `fis:sequential-thinking` skill — form hypotheses through structured reasoning, NOT guessing.
4. Spawn parallel `Explore` subagents to test each hypothesis against codebase evidence.
5. If 2+ hypotheses fail → auto-activate `fis:problem-solving` skill for alternative approaches.
...
5. **Acceptance criteria check:** Confirm every acceptance criterion from the issue / PR is now met by passing tests or manual proof before finalizing.
...
5. Run `/fis-journal` to write a concise technical journal entry upon completion
...
- `fis:scout` (Step 1) — understand before diagnosing
- `fis:debug` (Step 2) — systematic root cause investigation
- `fis:sequential-thinking` (Step 2) — structured hypothesis formation

**Conditional:**
- `fis:problem-solving` — auto-triggers when 2+ hypotheses fail in Step 2
- `fis:brainstorm` — multiple valid approaches, architecture decision (Deep only)
- `fis:context-engineering` — fixing AI/LLM/agent code
- `fis:project-management` — moderate+ for task hydration/sync-back
...
Load as needed:
- `references/selection-workflow.md` - AskUserQuestion format for mode
...
**Typically follows:** `/fis-debug` (after root cause analysis), `/fis-scout` (after locating affected code)
**Typically precedes:** `/fis-code-review` (review the fix), `/fis-test` (validate the fix)
**Related:** `/fis-craft` (alternative for feature work), `/fis-debug` (diagnose before fixing)
