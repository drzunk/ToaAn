...
```
/fis-team <template> <context> [flags]
```

**Templates:** `fis:research`, `fis:craft`, `fis:code-review`, `fis:debug`

**Flags:**
- `--devs N` | `--researchers N` | `--reviewers N` | `--debuggers N` -- team size
- `--plan-approval` / `--no-plan-approval` -- plan gate (default: on for craft)
- `--delegate` -- lead only coordinates, never touches code
- `--worktree` -- use git worktrees for implementation isolation (default: on for craft)
...
4. Do NOT fall back to subagents. `/fis-team` MUST use Agent Teams or abort.
...
- For craft Step 6 MERGE: spawn a dedicated merge teammate instead of lead doing it
...
Agent(
  ...
  prompt: "full instructions + FIS Context Block",
  model: "opus",                    # Required for Agent Teams teammates
  run_in_background: true,          # Non-blocking spawn
  isolation: "worktree"             # Git worktree isolation (craft devs)
)
```
...
```
FIS Context:
- Work dir: {FIS_PROJECT_ROOT or CWD}
- Reports: {FIS_REPORTS_PATH or "plans/reports/"}
- Plans: {FIS_PLANS_PATH or "plans/"}
- Branch: {FIS_GIT_BRANCH or current branch}
- Naming: {FIS_NAME_PATTERN or "YYMMDD-HHMM"}
- Active plan: {FIS_ACTIVE_PLAN or "none"}
...
## ON `/fis-team research <topic>` [--researchers N]:

*Wraps /fis-research skill -- scope, gather, analyze, report.*
...
3. **CALL** `TaskCreate` x N -- one per angle:
   ...
   - Description: `Investigate <angle> for topic: <topic>. Save report to: {FIS_REPORTS_PATH}/researcher-{N}-{FIS_NAME_PATTERN}-{topic-slug}.md. Format: Executive summary, key findings, evidence, recommendations. Mark task completed when done. Send findings summary to lead.`
...
4. **SPAWN** teammates x N via `Agent` tool:
   ...
   - `name: "researcher-{N}"`
   - Prompt: task description + FIS Context Block
...
6. **READ** all researcher reports from `{FIS_REPORTS_PATH}/`

7. **SYNTHESIZE** into: `{FIS_REPORTS_PATH}/research-summary-{FIS_NAME_PATTERN}-{topic-slug}.md`
...
11. **JOURNAL**: Run `/fis-journal` to write a concise technical journal entry upon completion

---

## ON `/fis-team craft <plan-path-or-description>` [--devs N]:

*Wraps /fis-craft skill -- plan, code, test, review, finalize.*
...
4. **SPAWN** developer teammates x N via `Agent` tool:
   ...
   - `name: "dev-{N}"`
   - Prompt: task description + plan context + FIS Context Block
...
7. **DOCS SYNC EVAL** (MANDATORY for craft -- from /fis-craft finalize):
   ```
...
10. **REPORT**: Tell user what was crafted, test results, docs impact.
11. **JOURNAL**: Run `/fis-journal` to write a concise technical journal entry upon completion

---

## ON `/fis-team review <scope>` [--reviewers N]:

*Wraps /fis-code-review skill -- scout, review, synthesize with evidence gates.*
...
3. **CALL** `TaskCreate` x N -- one per focus:
   ...
   - Description: `Review <scope> for <focus>. Output severity-rated findings only. Format: [CRITICAL|IMPORTANT|MODERATE] <finding> -- <evidence> -- <recommendation>. No "seems" or "probably" -- concrete evidence only. Save to: {FIS_REPORTS_PATH}/reviewer-{N}-{FIS_NAME_PATTERN}-{scope-slug}.md. Mark task completed when done.`
...
6. **SYNTHESIZE** into: `{FIS_REPORTS_PATH}/review-{scope-slug}.md`
...
10. **JOURNAL**: Run `/fis-journal` to write a concise technical journal entry upon completion

---

## ON `/fis-team debug <issue>` [--debuggers N]:

*Wraps /fis-fix skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
...
7. **WRITE** root cause report: `{FIS_REPORTS_PATH}/debug-{issue-slug}.md`
...
| Craft (4) | ~400K-800K | Highest cost -- code generation |
...
## Worktree Isolation (Craft Template)
...
> v3.0.0: Agent tool migration, worktree isolation for craft devs, run_in_background spawning, updated model requirements.
