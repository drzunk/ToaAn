...
**Date format:** Use `$FIS_PLAN_DATE_FORMAT` env var or default `YYMMDD-HHmm`.
...
When invoked directly with `/fis-project-organization [targets]`:
...
- `fis-journal` → docs/journals/
- `fis-craft` / `fis-fix` → source code paths (defer to language conventions)
- `fis-test` → tests/ structure
