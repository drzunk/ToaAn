# Red-Team Personas — fis:security Discovery Loop

Adapted from upstream `/autoresearch:security` adversarial lenses (uditgoenka/autoresearch, MIT).

Each persona represents a distinct attacker mindset. When `--red-team` mode is active, the discovery loop iterates over personas in order. Each iteration assumes the persona's threat model and probes from that lens before moving on. This is an **advisory** discovery tool — it reports findings; remediation and acceptance are the user's call.

---

## Persona Catalog (4 personas)
...
Phase 1: Security Adversary lens   → auth/injection/IDOR/disclosure
Phase 2: Supply Chain Attacker     → dependency tree, CI/CD, build artifacts
Phase 3: Insider Threat            → access control gaps, data export, audit trails
Phase 4: Infrastructure Attacker   → deployment config, secrets, SSRF, container security
Phase 5: STRIDE/OWASP sweep        → fill remaining OWASP A01–A10 + STRIDE S/T/R/I/D/E gaps
Phase 6: Consolidate + report      → dedupe across phases, rank by severity
```
...
6. **Chain** — prior findings compound (e.g. an open endpoint found in Phase 1 → tested for insider bulk-export in Phase 3)

### Results Log Extension (TSV gains a `persona` column)
...
3	insider-threat	bulk-export	Medium	A01	EoP	Likely	src/api/users.ts:80	GET /api/users no pagination limit
...
STRIDE: S[✓] T[✓] R[✗] I[✓] D[✓] E[✓] — 5/6
OWASP:  A01[✓] A02[✗] A03[✓] A04[✗] A05[✓] A06[✓] A07[✓] A08[✗] A09[✗] A10[✓] — 6/10
...
Any finding that references a secret value MUST mask it before logging:
...
Persona catalog and discovery loop pattern adapted from uditgoenka/autoresearch `references/security-workflow.md` § "Red-Team Adversarial Lenses" (MIT).
