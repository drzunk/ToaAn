...
**1. Chọn case** → chọn một màn, lọc Negative/Positive và thêm test case đề xuất →
...
| **login** | Có | `mvn -Plogin test` | Chỉ đăng nhập (+ Negative login trong `LoginTest`) |
...
| **discovery** | Có | `mvn -Pdiscovery test` | Quét field Negative → CSV (không assert) |
...
|---|---|---|
| **Dashboard · 1. Chọn case** (menu 9) | Cần đề xuất theo màn, Negative whitelist, merge nhanh vào danh sách chạy | Cần ma trận độ phủ đầy đủ (dùng mid/full) |
...
- Case **cấu hình** (loại đơn, untilStep, Negative form) → Sheet hoặc `local-cases.json` / Dashboard — **không** nhét thêm method vào Java test class.
